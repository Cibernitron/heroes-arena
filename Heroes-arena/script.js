...
if (
  ...
  "http://localhost/heroes-arena/Heroes-arena/pages/index.php"
) {
...
function ListenSearchBar() {
  searchBar.addEventListener("keyup", (e) => {
    if (searchBar.value.length > 0) {
      showName(searchBar.value).then((Response) => {
        if (!Response.result) {
...
function rotateCard(event) {
  ...
  if (
    ...
    if (event.currentTarget.classList.contains("is-flipped")) {
      shadow2.style.boxShadow = `${
        (mouseX - cardCenterX) * 0.35 - 1
      }px ${shadowThicknessY}px 10px 1px rgba(0, 0, 0, .6)`;
    } else {
    ...
    if (
      event.currentTarget.firstElementChild.classList.contains("is-flipped")
    ) {
      shadow2.style.boxShadow = `${
        (mouseX - cardCenterX) * 0.35 - 1
      }px ${shadowThicknessY}px 10px 1px rgba(0, 0, 0, .6)`;
...
function fillTheDescription(hero, heroHTML) {
  heroHTML.querySelector(".list-character-card__title").textContent =
    hero.hero_name;
  heroHTML.querySelector(".progress__val-atq").textContent = hero.hero_strength;
  heroHTML.querySelector(".progress__val-shield").textContent =
    hero.hero_combat;
  heroHTML.querySelector(".progress__val-speed").textContent = hero.hero_speed;
  heroHTML.querySelector(".progress__val-life").textContent =
    hero.hero_durability;
  heroHTML.querySelector(".bio__text-name").textContent = hero.hero_full_name;
  heroHTML.querySelector(".bio__text-alignment").textContent =
    hero.hero_alignment;
  heroHTML.querySelector(".bio__text-universe").textContent =
    hero.hero_publisher;
...
function displayNames(array) {
  ...
  array.forEach((element) => {
    ...
    buttonLiHeroes.addEventListener("click", (e) => {
      selectHero(element.id_hero).then((Response) => {
        if (!Response.result) {
          console.error("Problème avec la requête.");
          return;
        }
...
function addHero(hero, heroHtml) {
  ...
  heroHtml.querySelector(".list_img").src = hero.hero_sm;
...
function removeHero(heroHtml) {
  ...
  heroHtml.querySelector(
    ".list-character-card__title"
  ).textContent = `Choose Hero`;
  ...
  giveAllId()
    .then(async (Response) => {
      ...
      if (
        ...
        if (hero1 === undefined) {
          changeHeroWithDelay(50, hero1HTML).then((hero) => {
            if (hero) {
              hero1 = hero;
              addHero(hero1, hero1HTML);
            }
          ...
          changeHeroWithDelay(50, hero2HTML).then((hero) => {
            if (hero) {
              hero2 = hero;
              addHero(hero2, hero2HTML);
            }
...
function executeFight(attacker, defender) {
  ...
  if (damagesAttack > shieldDefense) {
    ...
    if (attacker === hero1) {
      ...
      setTimeout(() => {
        document
          .querySelector("#hero-container-1")
          .classList.add("hero__container-1");
        setTimeout(() => {
          if (defender.hero_durability > 0) {
            document
              .querySelector("#bam-hero2")
              .classList.remove("display-none");
          } else if (defender.hero_durability <= 0) {
            document
              .querySelector("#aargh-hero2")
              .classList.remove("display-none");
          }
          setTimeout(() => {
            document
      ...
      document
        .querySelector("#hero-container-1")
        .classList.remove("hero__container-1");
    } else if (attacker === hero2) {
      // if hero2 is attacker, image of hero go to hero1 and display bubble
      setTimeout(() => {
        document
          .querySelector("#hero-container-2")
          .classList.add("hero__container-2");
        setTimeout(() => {
          if (defender.hero_durability > 0) {
            document
              .querySelector("#bam-hero1")
              .classList.remove("display-none");
          } else if (defender.hero_durability <= 0) {
            document
              .querySelector("#aargh-hero1")
              .classList.remove("display-none");
          }
          setTimeout(() => {
            document
      ...
      document
        .querySelector("#hero-container-2")
        .classList.remove("hero__container-2");
    }
...
function battle(hero1, hero2) {
  ...
  const timer = setInterval(function () {
    ...
    hero1HTML.querySelector(".life-combat").style.width =
      Math.max(hero1LifeAfterDamage, 0) + `%`; // Reduce life-bar of hero 1 in terms of his remaining life
    hero2HTML.querySelector(".life-combat").style.width =
      Math.max(hero2LifeAfterDamage, 0) + `%`; // Reduce life-bar of hero 2 in terms of his remaining life
    hero1HTML.querySelector(
      ".progress__life-point"
    ).textContent = `${hero1.hero_durability}/${hero1Life}`;
    hero2HTML.querySelector(
      ".progress__life-point"
    ).textContent = `${hero2.hero_durability}/${hero2Life}`;
    ...
    if (attacker.hero_durability <= 0) {
      // If attacker have not life, defender win, and the fight is close
      winner = defender.hero_name;
      clearInterval(timer);
      resumeText.innerHTML += `<br><br><strong>${winner} gagne le combat !</strong>`;
    } else if (defender.hero_durability <= 0) {
      // If defender have not life, attacker win, and the fight is close
      winner = attacker.hero_name;
      clearInterval(timer);
      resumeText.innerHTML += `<br><br><strong>${winner} gagne le combat !</strong>`;
    }
    document.getElementById("selection-combat-text").scrollTop =
      document.getElementById("selection-combat-text").scrollHeight; // automatic scroll of text of fight
  }, speed2000);
}
...
function preparHeroToCombat(heroHTML) {
  ...
  heroHTML
    .querySelector(".hero__life-container")
    .classList.remove("display-none");
...
function preparCharacterCard(hero, heroHTML) {
  if (hero !== undefined) {
    heroHTML.querySelector(".progress__val-atq").style.width =
      hero.hero_strength + "%";
    heroHTML.querySelector(".progress__val-life").style.width =
      hero.hero_durability + "%";
    heroHTML.querySelector(".progress__val-shield").style.width =
      hero.hero_combat + "%";
    heroHTML.querySelector(".progress__val-speed").style.width =
      hero.hero_speed + "%";
  }
}
...
if (
  ...
  hero1HTML.querySelector(".hero__delete").addEventListener(
    "click",
    function (e) {
  ...
  hero2HTML.querySelector(".hero__delete").addEventListener(
    "click",
    function (e) {
  ...
  document
    .querySelector(".speed__selection")
